import React, { Component } from 'react';
import { View, StyleSheet, SafeAreaView, ScrollView, ImageBackground } from 'react-native';
import { Title, Avatar } from 'react-native-paper';
import FormInput from '../Componoents/FormInput';
import { SaveBtn, DeleteImageBtn, GoGalleryBtn, GoBackBtn } from '../Componoents/Buttons';
import * as ImagePicker from 'expo-image-picker';
import { GetDialog } from '../Componoents/GetDialog';
import { AlertHelper } from '../Componoents/Popup';
import { ReadUserData, ChangeProfile } from '../Componoents/UserProfile';
const options = { base64: true, quality: 1 };

class UpdateProfile extends Component{
  static navigationOptions = {
    title: 'weezit',
  }

  constructor(props){
    super(props);
    console.log('*** UpdateProfile Component ***');
    this.defaultImg = 'data:image/png;base64,';
    this.sender = this.props.navigation.state.params.sender;
    this.state = {
      visible: false,
      username: '',
      image: null,
      refresh: false,
      user: null
    }
  }

  setProfile = async() => {
    console.log('read user from db:')
    const user = await ReadUserData('Profile/');
    this.setState({ 
      user: user, 
      isLoading: false,
      });  
  }

  componentDidMount() {   
    if(this.state.user === null){
      this.setProfile();
    } 
    const { navigation } = this.props;
      this.focusListener = navigation.addListener('didFocus', () => {
      this.setProfile();
    });
  }


  saveChanges = async() => { 
    console.log('saveChanges function');
    const { username, image, user } = this.state;

    const data = {
      Id: user.ID,
      Name: username,
      Image: image
  }

    const response = await ChangeProfile(data);
    if(response == 200){
      AlertHelper.show('success', 'Success', 'Changes saved successfully!')
      this.props.navigation.navigate(this.props.navigation.navigate(this.sender))
    } else {
      AlertHelper.show('error', 'Error', 'Unfortunately the changes were not saved...')
    }
  }
 
  toGallery = async () => {
    console.log('toGallery function');
    try {
      let result = await ImagePicker.launchImageLibraryAsync(options);
      if (!result.cancelled) {
        const img = `data:image/jpg;base64,${result.base64}`
        this.setState({ image: img })
      }         
    } catch (E) {
      console.log(E);
    }
  }

  answerDialog = (e) => {
    console.log('answerDialog function');
    if(e == 'NO'){
      this.setState({ visible: false });
    }else {
      this.setState({ visible: false, image: this.defaultImg });
    }
  }

  toggleOverlay = () => this.setState((state) => { return { visible: !state.visible}});

    render(){
        return(
          <ImageBackground source={require('../Background/geometrical-halftone-dot-pattern-background-vector-design-from-circles-varying-sizes/1184.jpg')} style={{width: '100%', height: '100%'}}>
              <ScrollView>
              <SafeAreaView style={styles.container}>
              <Title style={styles.titleText}>Edit Yor Profile Details:</Title>
                
                <FormInput
                    username={this.state.username}
                    onChangeText={name => this.setState({username: name})}
                />
               
              <View style={styles.user}>
                    <Avatar.Image size={170} 
                                  source={{uri: this.state.image}} 
                                  style={styles.img}
                    />
              </View>

                <View style={styles.btnsContainer}>
                  <GoGalleryBtn onPress={() => this.toGallery()}/>
                  <DeleteImageBtn onPress={() =>this.toggleOverlay()}/>
                </View>
                
                <SaveBtn color='#26c6da' onPress={() => this.saveChanges()}/>
                <GoBackBtn color='#26c6da' onPress={() => this.props.navigation.navigate(this.sender)}/>
              </SafeAreaView>
            </ScrollView>
            
            <GetDialog 
              visible={this.state.visible} 
              title='Just to be sure,'
              description='Do you really want to delete your profile image?'
              onPress={this.answerDialog}/>

          </ImageBackground>
        )
    }
}
export default UpdateProfile;

const styles = StyleSheet.create({
  deleteBtn: {
    right:5
  },
  btnsContainer:{
    flexWrap: 'wrap',
    flexDirection: 'row',
    justifyContent: 'space-between',
    alignItems: 'center',
    margin: 20,
  },
  img: {
    alignItems: 'center',
    marginTop: 20,
  },
  container: {
    flex: 1,
    justifyContent: 'center',
    alignItems: 'center',
    paddingVertical: 20,
    flexDirection: 'column',
  },
  user: {
    flex: 1,
    justifyContent: 'center',
    alignItems: 'center',
    flexDirection: 'column',       
  },
  titleText: {
    fontSize: 24,
    marginBottom: 10,
    fontWeight: 'bold'
  },
});
